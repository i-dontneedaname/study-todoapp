...
type GetTasksReponse []TaskDTOResponse
...
func (h *TaskHTTPHandler) GetTasks(rw http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		responseHandler.ErrorResponse(err, "failed to get query params ('user_id' or 'limit' or 'offset') in HTTP handler")
	}
	...
	response := GetTasksReponse(tasksDTOFromDomains(tasksDomains))
